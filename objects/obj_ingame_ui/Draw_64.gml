/// @description 여기에 설명 삽입
// 이 에디터에 코드를 작성할 수 있습니다
draw_set_font(font_kor);
var dw = display_get_gui_width();
var dh = display_get_gui_height();

var marginX = 0.05;

var barPosX1 = dw * 0.85;
var barPosX2 = dw * 0.99;
var hpBarPosY1 = dh * 0.01;
var hpBarPosY2 = dh * 0.025;

var stmBarPosY1 = dh * 0.035;
var stmBarPosY2 = dh * 0.05;

var barMarginX = 38;
var barMarginY = 6;

// item
// 여기서 아이템 그리면 swap 1개 줄어듦
if (instance_exists(obj_player)) {
	for (var i = 0; i < array_length(global.myItems); i++) {
		var spr = global.myItemsSprite[$ global.myItems[i].Id];
		draw_sprite_ext(spr, 0, dw * (0.4 + marginX * i), dh * 0.06, 1, 1, 0, image_blend, 1);
	}
}
// key image
var key = draw_sprite_ext(spr_key, 0, dw - 50, 60, 0.9, 0.9, 0, image_blend, 1);

// text 한 번에 해서 swap 줄임
// key 개수
draw_text_transformed(dw - 28, 42, global.key, 1.5, 1.5, 0);
// hp text
draw_text_transformed(barPosX1 - barMarginX, hpBarPosY1 - barMarginY, "HP", 1, 1, image_angle);
// stamina text
draw_text_transformed(barPosX1 - barMarginX, stmBarPosY1 - barMarginY, "STM", 1, 1, image_angle);

if (instance_exists(obj_player)) {
	// hp bar
draw_healthbar(barPosX1, hpBarPosY1, barPosX2, hpBarPosY2, obj_player.myStats.hp, c_black, c_red, c_lime, 0, true, true);
// stamina bar
draw_healthbar(barPosX1, stmBarPosY1, barPosX2, stmBarPosY2, obj_player.myStats.stamina, c_black, c_dkgray, c_orange, 0, true, true);
}